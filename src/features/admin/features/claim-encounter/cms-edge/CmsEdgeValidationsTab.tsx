...
function ResolutionStatus({ status }: { status: RecordResolutionStatus }) {
	return (
		<span className="inline-flex items-center gap-1.5 text-xs font-medium">
			<span className={cn("size-2 rounded-full", RESOLUTION_STATUS_DOT[status])} />
			{status}
		</span>
	);
}
...
function InternalFileValidationPanel() {
	return (
		<CmsEdgeSectionPanel
			...
			footer={<TablePagination label="Showing 1 to 5 of 12 files" pages={[1, 2, 3, "ellipsis", 12]} />}
		>
				...
				<Table containerClassName={CMS_EDGE_TABLE_CONTAINER} className={CMS_EDGE_TABLE_CLASS}>
					<TableHeader>
						<TableRow className="hover:bg-transparent">
							...
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>File Type</TableHead>
							...
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "text-right")}>Records</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Status</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "text-right")}>Errors</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "text-right")}>Warnings</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Related Submission</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "pr-4 text-right")}>Action</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
						{CMS_EDGE_INTERNAL_FILE_VALIDATION.map((row) => (
							<TableRow key={row.id} className="border-b border-border/40 hover:bg-muted/20">
								...
								<TableCell className={CMS_EDGE_TABLE_CELL_CLASS}>{row.fileType}</TableCell>
								...
								<TableCell className={cn(CMS_EDGE_TABLE_CELL_CLASS, "text-right tabular-nums text-red-600")}>
									{formatCount(row.errors)}
								</TableCell>
								<TableCell className={cn(CMS_EDGE_TABLE_CELL_CLASS, "text-right tabular-nums text-amber-600")}>
			...
			footer={
				<>
					...
					<div className="flex flex-wrap gap-x-4 gap-y-1 border-t border-border/50 px-4 py-2 text-[10px] text-muted-foreground">
						{(["Open", "In Review", "Corrected", "Resubmitted", "Closed"] as const).map(
							(status) => (
				...
				<Table containerClassName={CMS_EDGE_TABLE_CONTAINER} className={CMS_EDGE_TABLE_CLASS}>
					<TableHeader>
						<TableRow className="hover:bg-transparent">
							...
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Error Code</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Error Description</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "text-right")}>Record Count</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Severity</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Related File</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Resolution Status</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "pr-4 text-right")}>Action</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
						{CMS_EDGE_INTERNAL_RECORD_VALIDATION.map((row) => (
							<TableRow key={row.id} className="border-b border-border/40 hover:bg-muted/20">
								<TableCell className={CMS_EDGE_TABLE_CELL_CLASS}>{row.recordType}</TableCell>
								<TableCell className={cn(CMS_EDGE_TABLE_CELL_CLASS, "font-mono font-medium")}>
									{row.errorCode}
								</TableCell>
								<TableCell className={cn(CMS_EDGE_TABLE_CELL_CLASS, "text-muted-foreground")}>
...
function ValidationTrendsPanel({ data, title }: { data: typeof CMS_EDGE_INTERNAL_VALIDATION_TREND; title: string }) {
	return (
		<CmsEdgeSectionPanel
			...
			<div className="min-h-[260px] flex-1 border-t border-border/50 px-3 py-3">
				<ResponsiveContainer width="100%" height="100%" minHeight={220}>
					<LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
						...
						<Legend iconSize={8} wrapperStyle={{ fontSize: 11, paddingBottom: 4 }} />
						<Line type="monotone" dataKey="passed" name="Passed" stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} />
						<Line type="monotone" dataKey="failed" name="Failed / Errors" stroke="#ef4444" strokeWidth={2} dot={{ r: 3 }} />
						<Line type="monotone" dataKey="warnings" name="Warnings" stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} />
					</LineChart>
				</ResponsiveContainer>
			</div>
		</CmsEdgeSectionPanel>
		...
		<CmsEdgeSectionPanel title="Top Error Categories (This Period)">
			<CmsEdgeTableScroll className="border-t border-border/50">
				<Table containerClassName={CMS_EDGE_TABLE_CONTAINER} className={CMS_EDGE_TABLE_CLASS}>
					<TableHeader>
						<TableRow className="hover:bg-transparent">
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Error Category</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "text-right")}>Errors</TableHead>
							<TableHead className={cn(CMS_EDGE_TABLE_HEAD_CLASS, "pr-4 text-right")}>% of Total</TableHead>
						</TableRow>
					</TableHeader>
					<TableBody>
						{CMS_EDGE_TOP_ERROR_CATEGORIES.map((row) => (
							<TableRow key={row.category} className="border-b border-border/40 hover:bg-muted/20">
								<TableCell className={CMS_EDGE_TABLE_CELL_CLASS}>{row.category}</TableCell>
								<TableCell className={cn(CMS_EDGE_TABLE_CELL_CLASS, "text-right tabular-nums")}>
									{formatCount(row.errors)}
								</TableCell>
...
function ExternalValidationSummaryPanel() {
	...
	return (
		<CmsEdgeSectionPanel
			...
			action={
				<StatusPill label="Validated by CMS" className="border-violet-200 bg-violet-50 text-violet-800" />
			}
			footer={
				<div className="border-t border-border/50 px-4 py-2.5 text-xs text-muted-foreground">
					<span>
						Total Files Submitted: {s.totalFilesSubmitted} | Total Records Returned:{" "}
						{formatCount(s.totalRecordsReturned)}
					</span>
					<span className="mt-1 block sm:mt-0 sm:float-right">
...
function ExternalFileValidationPanel() {
	return (
		<CmsEdgeSectionPanel
			...
			footer={<TablePagination label="Showing 1 to 5 of 12 files" pages={[1, 2]} />}
		>
			<CmsEdgeTableScroll className="border-t border-border/50">
				<Table containerClassName={CMS_EDGE_TABLE_CONTAINER} className={CMS_EDGE_TABLE_CLASS}>
					<TableHeader>
						<TableRow className="hover:bg-transparent">
							...
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>File/Report Name</TableHead>
							<TableHead className={CMS_EDGE_TABLE_HEAD_CLASS}>Related Submission</TableHead>
...
function ExternalQuickActionsPanel() {
	const maxCount = Math.max(...CMS_EDGE_EXTERNAL_ERROR_BREAKDOWN.map((item) => item.count));
			...
			<div className="grid gap-4 border-t border-border/50 p-4 lg:grid-cols-[minmax(0,220px)_minmax(0,1fr)]">
				...
				<div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
					{CMS_EDGE_EXTERNAL_QUICK_ACTIONS.map((action) => {
						const Icon = EXTERNAL_QUICK_ACTION_ICONS[action.id] ?? ClipboardList;
						return (
							<button
								key={action.id}
								type="button"
...
export function CmsEdgeValidationsTab() {
	...
	return (
		<div className={CMS_EDGE_PAGE_STACK}>
			<Tabs value={subTab} onValueChange={(v) => setSubTab(v as ValidationSubTab)}>
