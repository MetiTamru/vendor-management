...
export async function listDomains() {
	return withMockOrRemote(() => mock.EXECUTIVE_DOMAINS, async () => []);
}

export async function listAlerts() {
	return withMockOrRemote(() => mock.EXECUTIVE_ALERTS, async () => []);
}

export async function listRegulatoryQualityMeasures() {
	return withMockOrRemote(() => regulatoryMock.RQ_QUALITY_MEASURES, async () => []);
}

export async function listRiskExceptions() {
	return withMockOrRemote(() => riskMock.RE_TOP_EXCEPTIONS, async () => []);
}
